...
from my.tensorflow.rnn import bidirectional_dynamic_rnn, my_bidirectional_dynamic_rnn,my_new_bidirectional_dynamic_rnn
...
class Model(object):
    def __init__(self, config, scope, rep=True):
        ...
        N, M, JX, JQ, VW, VC, W = \
            ...
            config.max_ques_size, config.len_new_emb_mat, config.char_vocab_size, config.max_word_size
        ...
        self.pointer_gen = tf.placeholder('bool', [], name='pointer_gen')
        ...
        # Loss outputs
        self.loss_task1 = None
        self.loss_task2 = None
    ...
    def _build_forward(self):
            ...
            if config.use_word_emb:
                with tf.variable_scope("emb_var"), tf.device("/cpu:0"):
                    if config.mode == 'train':
                        ...
                        word_emb_mat = tf.get_variable("word_emb_mat", dtype='float', shape=[VW, dw], initializer=init_word_emb)
        ...
        with tf.variable_scope("prepro"):
            ...
            if config.share_lstm_weights:
                tf.get_variable_scope().reuse_variables()
                (fw_h, bw_h), (fw_s,bw_s) = bidirectional_dynamic_rnn(cell_fw, cell_bw, xx, x_len, dtype='float', scope='u1')  # [N, M, JX, 2d]
        ...
        with tf.variable_scope("main"):
            ...
            (fw_g2, bw_g2), _ = bidirectional_dynamic_rnn(d_cell4_fw, d_cell4_bw,tf.concat(axis=3, values=[p0, g1, a1i, g1 * a1i]), x_len, dtype='float', scope='g2')  # [N, M, JX, 2d]
            ...
            logits2 = get_logits([g2, p0], d, True, wd=config.wd, input_keep_prob=config.input_keep_prob,
                                 mask=self.x_mask,is_train=self.is_train, func=config.answer_func, scope='logits2')
            ...
            with tf.variable_scope("rnn_decoder",reuse=tf.AUTO_REUSE):
                ...
                W_dense = tf.get_variable(name="W_dense",shape =[2 * nodes, tgt_vocab_size], dtype=tf.float32,initializer=init)
                b_dense = tf.get_variable(name ="b_dense",shape=[tgt_vocab_size], dtype=tf.float32,initializer=tf.zeros_initializer)

                W_att_dec = tf.get_variable(name = "W_att_dec",shape=[2 * nodes, 2 * nodes], dtype=tf.float32,initializer=init)
                W_att_enc = tf.get_variable(name="W_att_enc1",shape=[1, 1, 2 * nodes, 2 * nodes], dtype=tf.float32,initializer=init)
                v_blend = tf.get_variable(name= "v_blend",shape=[1, 2 * nodes], dtype=tf.float32,initializer=init)

                pad_time_slice = tf.fill([N], 0, name='PAD')
                pad_step_embedded = tf.nn.embedding_lookup(word_emb_mat, pad_time_slice)
                ...
                def execute_pointer_network(attn_dist):
                    ...
                    index_pos = tf.concat([tf.reshape(tf.range(start=0,limit=N,dtype=tf.int64),[N,1]),tf.zeros([N,1],tf.int64),index_pos],axis=1)
                    index_pos = tf.cast(tf.gather_nd(params=self.x,indices=index_pos),dtype=tf.int64)
                    return index_pos
                ...
                def loop_fn_initial():
                    initial_elements_finished = (0 >= self.target_sequence_length)  # all False at the initial step
                    ...
                    initial_cell_output = tf.cond(self.pointer_gen, lambda: tf.zeros([M*JX],tf.float32),lambda : tf.zeros([2*nodes],tf.float32))
                ...
                def loop_fn_transition(time, previous_output, previous_state, previous_loop_state):
                    def get_next_input():
                        ...
                        encoder_features = tf.nn.conv2d(encoder_output, W_att_enc, [1, 1, 1, 1], "SAME")  # shape (batch_size,max_enc_steps,1,attention_vec_size)
                        ...
                        attn_dist = tf.nn.softmax(e_not_masked)  # (batch_size, max_enc_steps)
                        attn_dist = tf.Print(attn_dist, [tf.shape(attn_dist)], message="attn_dist",
                                          first_n=5, summarize=200)

                        #Multiplying all the 2d vectors with same attn_dist values,and finally keeping 1 2d vector for every batch example
                        context_vector = tf.reduce_sum(tf.reshape(attn_dist, [N, -1, 1, 1]) * encoder_output,[1, 2])  # shape (batch_size, attn_size).
                        ...
                        #output_logits = tf.add(tf.matmul(previous_output, W_dense), b_dense)
                        prediction = tf.cond(self.pointer_gen, lambda : execute_pointer_network(attn_dist),
                                             lambda: execute_normal_decoder(previous_output,W_dense,b_dense))

                        with tf.variable_scope("modified_dec_inputs",reuse=tf.AUTO_REUSE):
                            next_input = tf.cond(self.is_train,
                                             lambda :_linear(args=[context_vector] +[tf.reshape(decoder_emb_inp[:, time], (N, dw))],output_size=dw, bias=True),
                                             lambda :_linear([context_vector] +[tf.nn.embedding_lookup(word_emb_mat, prediction)],dw,True))

                        return next_input,attn_dist

                    elements_finished = (time >= self.target_sequence_length)  # this operation produces boolean tensor of [batch_size]
                    # defining if corresponding sequence has ended
                    finished = tf.reduce_all(elements_finished)  # -> boolean scalar
                    #input = tf.cond(finished, lambda: tf.concat([pad_step_embedded, my_encoder_final_state_h], 1),get_next_input)
                    input, attn_distribution = tf.cond(finished, lambda: (pad_step_embedded,tf.zeros([N, M*JX],tf.float32)),get_next_input)
                    attn_distribution = tf.Print(attn_distribution, [tf.shape(attn_distribution)], message="attn_distribution",
                                         first_n=5, summarize=200)
                    state = previous_state
                    output = tf.cond(self.pointer_gen, lambda: attn_distribution, lambda  : previous_output)
                ...
                decoder_outputs = tf.Print (decoder_outputs,[tf.shape(decoder_outputs)],message="decoder_outputs", first_n=5, summarize=200)
                ...
                decoder_logits_flat = tf.cond(self.pointer_gen,lambda: decoder_outputs_flat,
                                              lambda: tf.add(tf.matmul(decoder_outputs_flat, W_dense), b_dense))
                decoder_logits = tf.cond(self.pointer_gen,lambda : tf.reshape(decoder_logits_flat, (decoder_max_steps, decoder_batch_size, decoder_dim)),
                                         lambda: tf.reshape(decoder_logits_flat, (decoder_max_steps, decoder_batch_size, tgt_vocab_size)))
    ...
    def _build_loss(self):
        ...
        if config.mode == 'train':
            #returns loss per training example
            crossent = tf.nn.sparse_softmax_cross_entropy_with_logits(labels=self.decoder_targets, logits=self.decoder_logits_train)
            #sums the loss across all examples
            train_loss = tf.reduce_sum(crossent * self.target_weights) / tf.cast(N, dtype=tf.float32)

            losses = tf.nn.softmax_cross_entropy_with_logits(
                logits=self.index_start, labels=tf.cast(tf.reshape(self.y, [-1, M * JX]), 'float'))
            losses2 = tf.nn.softmax_cross_entropy_with_logits(
                logits=self.index_end, labels=tf.cast(tf.reshape(self.y2, [-1, M * JX]), 'float'))
            ce_loss = tf.reduce_mean(loss_mask * losses)
            ce_loss2 = tf.reduce_mean(loss_mask * losses2)
            tf.add_to_collection('losses_task2', ce_loss)
            tf.add_to_collection("losses_task2", ce_loss2)
        else:
            train_loss=tf.zeros([1],tf.int32)

        tf.add_to_collection('losses_task1', train_loss)

        print ("LOSSES",tf.get_collection('losses_task1', scope=self.scope))
        self.loss_task1 = tf.add_n(tf.get_collection('losses_task1', scope=self.scope), name='loss') # loss for the entire batch ?
        self.loss_task2 = tf.add_n(tf.get_collection('losses_task2', scope=self.scope), name='loss') # loss for the entire batch ?

        tf.summary.scalar(self.loss_task2.op.name, self.loss_task2)
        tf.add_to_collection('ema/scalar', self.loss_task2)
    ...
    def _build_ema(self):
        ...
        with tf.control_dependencies([ema_op]):
            self.loss_task2 = tf.identity(self.loss_task2)
    ...
    def get_loss_task1(self):
        return self.loss_task1

    def get_loss_task2(self):
        return self.loss_task2
    ...
    def get_feed_dict(self, batch, is_train, supervised=True):
        ...
        answer_sos = np.zeros([N, JX], dtype='int32')
        ...
        feed_dict[self.pointer_gen] = config.pointer_network
        feed_dict[self.decoder_inputs]=answer_sos
        ...
        if supervised:
            ...
            span_list=[]
            for i, spani in enumerate(batch.data['spans']):
                j_start, k_start = spani[0]
                y[i, 0, k_start] = True
                j_end, k_end = spani[1]
                y2[i, 0, k_end] = True
                span_list.append([[j_start, k_start],[j_end, k_end]])

            print ("spans vector",span_list)
        ...
        def find_pos_in_summary(word,summary):
            try:
                return summary.index(word.lower())
            except:
                return len(summary)-1

        def check_sanity(x_summ):
            d=batch.shared['idx2word']
            x_str = ' '.join(d[id] for id in x_summ)
            print("RETRIEVED SUMM",x_str)
        ...
        for i, ansi in enumerate(batch.data['answerss']):
            ...
            for k, ansik in enumerate(ansi[1]):#answer with eos
                if k == config.max_sent_size:
                    break
                if config.pointer_network:
                    #this is to convert answers to lists of positions in the summary, to be used for loss function
                    if ansik == "</s>":
                        answer_eos[i, k] = len(X[i][0]) -1
                    else:
                        answer_eos[i,k] = find_pos_in_summary(ansik,X[i][0])
                else:
