...
class DataSet(object):
    ...
    def _sort_key(self, idx):
        answer = self.data['answerss'][idx][0]
        return len(answer)

    def _sort_key_2(self, idx):
        rx = self.data['*x'][idx]
        x = self.shared['x'][rx[0]][0]
    ...
    def get_batches(self, batch_size, num_batches=None, shuffle=False, cluster=False):
        ...
        if shuffle:
            ...
            if cluster:
                ...
                sorted_idxs_2 = sorted(sorted_idxs, key=self._sort_key_2)
                sorted_grouped = lambda: list(grouper(sorted_idxs_2, batch_size))
    ...
    def get_multi_batches(self, batch_size, num_batches_per_step, num_steps=None, shuffle=False, cluster=False):
        batch_size_per_step = batch_size * num_batches_per_step #num_batches_per_step = num_gpus =1
...
def read_data(config, data_type, ref, data_filter=None):
    ...
    #entity_dict = json.load(open('/Users/dhruv100691/Documents/cs546/CS-546--Narrative-QA/bi-att-flow-dev/entity_dict_new_method.json', 'r'))
    #entities_set = set()
    #for k,v in entity_dict.items():
    #    for key in v.keys():
    #        entities_set.add(key.lower())
    #print (len(entities_set),entities_set)
    ...
    if not ref:
        ...
        if config.finetune:
            ...
            #shared['word2idx'] = {word: idx + 4 for idx, word in
            #                      enumerate(word for word, count in word_counter.items()
            #                                if count > config.word_count_th and word not in word2vec_dict and word not in entities_set)}
            #len_sh = len(shared['word2idx'])
            #shared['word2idx'].update({word: idx + 4 +len_sh for idx, word in
            #                      enumerate(word for word, count in word_counter.items()
            #                                if count > config.word_count_th and word not in word2vec_dict and word in entities_set)})
            shared['word2idx'] = {word: idx + 4 for idx, word in
                                       enumerate(word for word, count in word_counter.items()
        ...
        #config.vw_wo_entity_size = len_sh+4
        config.vw_wo_entity_size = 0
        #json.dump({'word2idx': shared['word2idx'], 'char2idx': shared['char2idx'],'idx2word': shared['idx2word']}, open(shared_path, 'w'))
        ...
        for key, val in new_shared.items():
            ...
            if key == 'new_word2idx':
                word2id_dict = shared['new_word2idx']
                shared['new_word2idx'] = {word:int(id) for word,id in word2id_dict.items()}

        #cocatenated vocab for dev and train sets to allow for training our own embeddings
        print ("IN DEV, shared word2id",len(shared['new_word2idx']),len(shared['word2idx']))
        word2vec_dict = shared['lower_word2vec'] if config.lower_word else shared['word2vec']
        new_word2idx_dict = {word: idx + len(shared['new_word2idx']) for idx, word in
                             enumerate(word for word in word2vec_dict.keys() if word not in shared['word2idx']
                                       and word not in shared['new_word2idx'])}
        idx2vec_dict = {idx - len(shared['new_word2idx']) : word2vec_dict[word] for word, idx in new_word2idx_dict.items()}
        shared['new_word2idx'].update(new_word2idx_dict)
        new_emb_mat = np.array([idx2vec_dict[idx] for idx in range(len(idx2vec_dict))], dtype='float32')
        shared['new_emb_mat'] = np.concatenate((shared['new_emb_mat'],new_emb_mat))
        shared['idx2word'].update({id + len(shared['word2idx']): word for word, id in shared['new_word2idx'].items()})
...
def update_config(config, data_sets):
    ...
    print ("emb size",config.word_emb_size)
    if config.single:
