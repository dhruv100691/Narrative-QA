...
def my_bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs, sequence_length=None,
    ...
    flat_inputs=tf.Print(flat_inputs,[tf.shape(flat_inputs)],message="flat inputs g1 shape:",first_n=5)
    flat_len = None if sequence_length is None else tf.cast(my_flatten_2(sequence_length), 'int64')
    flat_len=tf.Print(flat_len,[flat_len],message="flat lebgths of g1:",first_n=5)
...
def my_new_bidirectional_dynamic_rnn(cell_fw, cell_bw, inputs, sequence_length=None,
                              initial_state_fw=None, initial_state_bw=None,
                              dtype=None, parallel_iterations=None,
                              swap_memory=False, time_major=False, scope=None):
    assert not time_major
